...
int main(int argc, char *argv[]) {
    ...
    for (int iterazioni = 0; iterazioni < 10; iterazioni++) {
        ...
        for (int iterazioni = 0; iterazioni < 10; iterazioni++) {
            ...
            for (const pixel_AoS& pC : centers) {
                ...
                for (int i = pC.y > (int)S ? pC.y - (int)S : 0; i < iMax; i++) {
                    for (int j = pC.x > (int)S ? pC.x - (int)S : 0; j < jMax; j++) {
                        ...
                        pixel_AoS p = { labMin[0],labMin[1] ,labMin[2], j, i};
            ...
            for (int k = 0; k < spCounter; k++) {
                ...
                pixel_AoS newC = {(l_avg / pixelCounter), (a_avg / pixelCounter), (b_avg / pixelCounter), static_cast<int>(std::round(x_avg / pixelCounter)), static_cast<int>(std::round(y_avg / pixelCounter))};
        ...
        time_results_avg += static_cast<double>(end - start) / CLOCKS_PER_SEC;
    ...
    printf( "Tempo medio: %.4f", time_results_avg / 10);
