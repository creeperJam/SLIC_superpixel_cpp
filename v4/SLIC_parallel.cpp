...
image_SoA run_parallel(const image_SoA &image_SoA, const int &k) {
    ...
    parClustersInitialization(image_SoA, clusters_centers, step, k);
    const int real_k = static_cast<int>(clusters_centers.L.size());
    std::vector<std::mutex> row_mutexes(height);

    parClustersPerturbation(image_SoA, clusters_centers);
    ...
    for (int iterazioni = 0; iterazioni < 10; iterazioni++) {
        parBestMatchPixelNeighborhood(image_SoA, clusters_centers, distances, labels, step, row_mutexes);
        parNewClustersCenters(image_SoA, clusters_centers, labels, error, real_k);
    }
...
void parBestMatchPixelNeighborhood(const image_SoA& img, const pixels_SoA& clusters_centers, Mat& distances, Mat& labels, const float& step, std::vector<std::mutex>& row_mutexes) {
    int num_centers = static_cast<int>(clusters_centers.L.size());
...
#pragma omp parallel for schedule(dynamic, 4)
    for (int cc_index = 0; cc_index < num_centers; ++cc_index) {
        ...
        int current_cluster_id = cc_index;

        int r_min = std::max(0, pixelc_x - static_cast<int>(step));
        int r_max = std::min(height, pixelc_x + static_cast<int>(step));
        int c_min = std::max(0, pixelc_y - static_cast<int>(step));
        int c_max = std::min(width, pixelc_y + static_cast<int>(step));
        ...
        for (int i = r_min; i < r_max; i++) {
            ...
            for (int j = c_min; j < c_max; ++j) {
                ...
                float dL = img.L[idx] - pixelc_l;
                float da = img.a[idx] - pixelc_a;
                float db = img.b[idx] - pixelc_b;
...
void parNewClustersCenters(const image_SoA& img, pixels_SoA& clusters_centers, const Mat& labels, double& error, const int& real_k) {
    ...
    #pragma omp parallel for schedule(dynamic, 4) reduction(+:error)
