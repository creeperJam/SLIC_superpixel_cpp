...
// Definizione della posizione iniziale dei centri di tutti i cluster
void parClustersInizialization(const Mat& img, std::vector<pixel_AoS>& clusters_centers, const float& step, const int& k) {
...
void parClustersPerturbation(const Mat& img, std::vector<pixel_AoS>& clusters_centers) {
...
void parBestMatchPixelNeighborhood(Mat& img, const std::vector<pixel_AoS>& clusters_centers, Mat& distances, Mat& labels, const float& step) {
...
#pragma omp parallel for schedule(dynamic)
    for (int k = 0; k < num_centers; ++k) {
        const pixel_AoS& pixel_c = clusters_centers[k];
...
void parNewClustersCenters(const Mat& img, std::vector<pixel_AoS>& clusters_centers, const Mat& labels, double& errore_finale, const int& real_k) {
